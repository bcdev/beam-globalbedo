 ...
 * This version 2 is based on the new accumulation setup without file I/O.
 *
 * @author Olaf Danne
 * @version $Revision: $ $Date:  $
 */
@OperatorMetadata(alias = "ga.inversion.inversion2",
        description = "Performs final inversion from fully accumulated optimal estimation matrices",
        authors = "Olaf Danne",
        version = "1.0",
        copyright = "(C) 2011 by Brockmann Consult")

public class Inversion2Op extends PixelOperator {
...
//    private float[][] daysToTheClosestSampleArray;
    ...
    protected void computePixel(int x, int y, Sample[] sourceSamples, WritableSample[] targetSamples) {
        ...
        if (fullAccumulator != null && maskAcc > 0 && ((usePrior && maskPrior > 0) || !usePrior)) {
            ...
            daysToTheClosestSample = daysToTheClosestSampleArray[x][y];
        } else {
            if (maskPrior > 0.0) {
                if (usePrior) {
