...
/**
 * Implementation of the daily accumulation part.
 * This is a special 'single pixel' version creating a target product of size 1x1.
...
@OperatorMetadata(alias = "ga.inversion.dailyacc.single",
        description = "Provides daily accumulation of single BBDR observations for a single pixel",
        authors = "Olaf Danne",
        version = "1.0",
        copyright = "(C) 2011 by Brockmann Consult")
public class DailyAccumulationSinglePixelOp extends PixelOperator {
    ...
    protected void configureTargetProduct(ProductConfigurer productConfigurer) throws OperatorException {
        ...
        // todo: now we need a real target product!!
    ...
    private void fillTargetSamples(WritableSample[] targetSamples, Accumulator accumulator) {
        for (int i = 0; i < 3 * AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS; i++) {
            for (int j = 0; j < 3 * AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS; j++) {
                targetSamples[TRG_M[i][j]].set((float) accumulator.getM().get(i, j));
            }
        }
        for (int i = 0; i < 3 * AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS; i++) {
            targetSamples[TRG_V[i]].set((float) accumulator.getV().get(i, 0));
        }
        targetSamples[TRG_E].set((float) accumulator.getE().get(0, 0));
    ...
    public static class Spi extends OperatorSpi {
        ...
        public Spi() {
            super(DailyAccumulationSinglePixelOp.class);
        }
    }
}
