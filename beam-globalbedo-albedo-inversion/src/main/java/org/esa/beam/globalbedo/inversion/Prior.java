...
public class Prior {
    ...
    public static Prior extractPriorFromProduct(int x, int y, Tile[] priorTiles, double priorScaleFactor) {
        ...
        double mask = 0.0;
        final int priorIndexNsamples = 2 * AlbedoInversionConstants.NUM_ALBEDO_PARAMETERS;
        double nSamples = AlbedoInversionUtils.checkSummandForNan(priorTiles[priorIndexNsamples].getSampleDouble(x,y));
        ...
        for (int i = 0; i < AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS; i++) {
            for (int j = 0; j < AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS; j++) {
                final int priorIndexMij = AlbedoInversionConstants.NUM_BBDR_WAVE_BANDS * i + j;
                final double m_ij = priorTiles[priorIndexMij].getSampleDouble(x,y);
                priorMean.set(index, 0, m_ij);
                final int priorIndexSDij = Inversion2Op.PRIOR_OFFSET + priorIndexMij;
                final double sd_ij = priorTiles[priorIndexSDij].getSampleDouble(x,y);
                priorSD.set(index, 0, sd_ij);
                if (priorMean.get(index, 0) > 0.0 && priorSD.get(index, 0) == 0.0) {
                    mask = 1.0;
                    nSamples = 1.E-20;
