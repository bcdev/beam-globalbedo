 ...
 * @author akheckel
 */
@OperatorMetadata(alias = "ga.ProbavPrepOp",
                  description = "Create Probav product for input to Globalbedo aerosol retrieval and BBDR/SDR processor",
                  authors = "Olaf Danne",
                  version = "1.0",
                  copyright = "(C) 2015 by Brockmann Consult")
public class ProbavPrepOp extends Operator {
    ...
    public void initialize() throws OperatorException {
        ...
        final boolean needSurfacePres = (!sourceProduct.containsBand(instrC.getSurfPressureName("PROBAV")));

        Product szaSubProduct = sourceProduct;

        // subset might have set ptype to null, thus:
        if (szaSubProduct.getDescription() == null) szaSubProduct.setDescription("probav product");
...
//            idepixProduct = GPF.createProduct(OperatorSpi.getOperatorAlias(GlobAlbedoOp.class), pixelClassParam, szaSubProduct);
        ...
        if (needSurfacePres){
            ...
            surfPresBand = new VirtualBand(instrC.getSurfPressureName("PROBAV"),
