...
def ProcessInversion(M_acc, V_acc, E_acc, Mask_acc, Lambda, PriorFile, columns, rows, UsePrior, DoYClosestSample, tile, year, Snow, Invalid=-9999):
...
if Snow == 1:
    prior_files_path = DataDir + '/Priors/' + tile + '/background/processed.p1.0.618034.p2.1.00000/Kernels.' + PriorDoY + '*.005.' + tile + '.backGround.Snow'
else:
    prior_files_path = DataDir + '/Priors/' + tile + '/background/processed.p1.0.618034.p2.1.00000/Kernels.' + PriorDoY + '*.005.' + tile + '.backGround.NoSnow'
...
while Processes or PriorFileToProcess:
    ...
    if (len(Processes) < NumProcesses) and PriorFileToProcess:
        p = multiprocessing.Process(target=ProcessInversion, args=[ data.MData[i], data.VData[i], data.EData[i], data.Mask[i], Lambda, PriorFileToProcess[0], columns, rows, UsePrior, data.DoYClosestSample[i], tile, year, Snow])
