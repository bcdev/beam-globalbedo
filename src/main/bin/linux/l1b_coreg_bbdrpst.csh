...
# example L1b --> BBDR MERIS: nohup ./l1b_coreg_bbdrpst.csh /data/globalbedo/seaice/L1b/MERIS/ /data/globalbedo/seaice/L1b/AATSR /data/globalbedo/seaice/20070621/fulldaytest MERIS > fulldaytest_MERIS.log &

# now includes coregistration, 20130529
...
#echo "time $BEAMDIR/bin/gpt_8192.sh ga.seaice.merisaatsr.colloc -PdoCoregistration=true -PmasterSensor=$bbdrSensor -PmasterInputDataDir=$masterInputDataDir -PslaveInputDataDir=$slaveInputDataDir -PcollocOutputDataDir=$collocDir"
#time $BEAMDIR/bin/gpt_8192.sh ga.seaice.merisaatsr.colloc -PdoCoregistration=true -PmasterSensor=$bbdrSensor -PmasterInputDataDir=$masterInputDataDir -PslaveInputDataDir=$slaveInputDataDir -PcollocOutputDataDir=$collocDir

foreach SRCFILE (`ls ${masterInputDataDir}/*.N1`)
    set SRCBASE = `basename ${SRCFILE} |cut -d'.' -f1`
    echo "Coregister and collocate from MERIS L1b product " ${SRCFILE} "..."
    echo "time ${BEAMDIR}/bin/gpt_8192.sh ga.seaice.merisaatsr.coreg -SsourceProduct=${SRCFILE} -PslaveInputDataDir=$slaveInputDataDir -PcollocOutputDataDir=$collocDir"
    time ${BEAMDIR}/bin/gpt_8192.sh ga.seaice.merisaatsr.coreg -SsourceProduct=${SRCFILE} -PslaveInputDataDir=$slaveInputDataDir -PcollocOutputDataDir=$collocDir
    echo "Done coregistration and collocation of product " ${SRCFILE} "."
end
