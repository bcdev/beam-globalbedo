...
bands=['BHR_NIR','BHR_SW','BHR_VIS','WNSamples','BHR_SW_CoV','RelEntropy']
...
m = PMonitor(inputs, 
             request='ga-l3-staging-browse2mov-monthly', 
             logdir='log',
             hosts=[('localhost',64)],
    ...
    for snowMode in snowModes:
        for res in resolutions:
            for proj in projections:
                for band in bands:
                    stagingMoviesInputDir = gaRootDir + '/staging/QL/albedo_monthly/' + snowMode + '/' + year + '/' + res + '/' + proj + '/' + band
                    stagingMoviesResultDir = gaRootDir + '/staging/Movies/albedo_monthly/' + snowMode + '/' + year + '/' + res + '/' + proj
