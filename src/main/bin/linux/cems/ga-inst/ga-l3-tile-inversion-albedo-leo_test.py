...
#
# TEST VERSION: per PMonitor execute of ga-l3-tile-inversion-dailyacc-leo_test-step.sh,
# ga-l3-tile-inversion-albedo-leo_test-step.sh, many LSF jobs are initiated.
#
...
#m = PMonitor(inputs, 
#             request='ga-l3-tile-inversion-albedo-leo_test',
#             logdir='log', 
#             hosts=[('localhost',192)], # let's try this number...
#             types=[ ('ga-l3-tile-inversion-dailyacc-leo_test-step.sh',192), 
#                     ('ga-l3-tile-inversion-albedo-leo_test-step.sh',192)] )

m = PMonitor(inputs,
             request='ga-l3-tile-inversion-albedo-leo_test',
             logdir='log',
             hosts=[('localhost',96)], # let's try this number for 1200x1200 LEO...
             types=[ ('ga-l3-tile-inversion-dailyacc-leo_test-step.sh',72),
                     ('ga-l3-tile-inversion-albedo-leo_test-step.sh',24)] )
...
for year in years:
    ...
    for tile in tiles:
        ...
        if isPolarTile(tile):
            ...
            startDoy='273'
        m.execute('ga-l3-tile-inversion-dailyacc-leo_test-step.sh', ['bbdrs'], [left_accs_name], parameters=[tile,leftyear,startDoy,endDoy,step,modisTileScaleFactor,gaRootDir,bbdrRootDir,beamDir])
        ...
        if isPolarTile(tile):
            ...
            endDoy = '097'
        m.execute('ga-l3-tile-inversion-dailyacc-leo_test-step.sh', ['bbdrs'], [right_accs_name], parameters=[tile,rightyear,startDoy,endDoy,step,modisTileScaleFactor,gaRootDir,bbdrRootDir,beamDir])

        # center year daily accs (after completion of wings):
        startDoy = '000'
        endDoy = '361'
        wing_accs_names = [left_accs_name, right_accs_name]
        #m.execute('ga-l3-tile-inversion-dailyacc-leo_test-step.sh', wing_accs_names, [center_accs_name], parameters=[tile,year,startDoy,endDoy,step,modisTileScaleFactor,gaRootDir,bbdrRootDir,beamDir])
        m.execute('ga-l3-tile-inversion-dailyacc-leo_test-step.sh', ['bbdrs'], [center_accs_name], parameters=[tile,year,startDoy,endDoy,step,modisTileScaleFactor,gaRootDir,bbdrRootDir,beamDir])
        ...
        m.execute('ga-l3-tile-inversion-albedo-leo_test-step.sh', all_accs_names, ['dummy1'], parameters=[tile,year,startDoy,endDoy,gaRootDir,bbdrRootDir,inversionRootDir,usePrior,priorDir,beamDir,modisTileScaleFactor,albedoDir])
