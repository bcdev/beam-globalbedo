...
#from matplotlib.colors import NP_CLIP_creationYear='2014'
...
arrayTab[1]=np.where(arrayTab[1]!=0, arrayTab[1],-1.0)

dat=np.divide(arrayTab[0],arrayTab[1])

dat=np.where(dat<=max[0], dat, max[0])
dat=np.where(dat>min[0], dat, 0)
...
plotImag(dat,outdir+'/'+name+"_"+bandDisplay[0]+".png", bandDisplay[0], dateStr, tickets, cdict)
