...
for tile in tiles:
    for doy in doys:
        ...
        brdfProduct = brdfDir + '/GlobAlbedo.brdf.merge.' + year + doy + '.' + tile + '.nc'
	brdfProducts.append(brdfProduct)

m = PMonitor(brdfProducts, 
             request='ga-l3-tile-brdf-convert-nc-to-dimap',
             logdir='log', 
             hosts=[('localhost',192)], # let's try this number...
             types=[ ('ga-l3-tile-brdf-convert-nc-to-dimap-step.sh',192)] )
...
for tile in tiles:
    ...
    for doy in doys:
        ...
        brdfProduct = brdfDir + '/GlobAlbedo.brdf.merge.' + year + doy + '.' + tile + '.nc'
        brdfDimapProduct = brdfDir + '/GlobAlbedo.brdf.merge.' + year + doy + '.' + tile + '.dim'

        m.execute('ga-l3-tile-brdf-convert-nc-to-dimap-step.sh', [brdfProduct], [brdfDimapProduct], parameters=[tile,year,doy,gaRootDir,beamDir,brdfDir])
