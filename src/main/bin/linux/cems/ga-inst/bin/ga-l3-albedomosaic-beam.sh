...
then
    echo "time $beamRootDir/bin/gpt-d-l3.sh ga.albedo.albedo -PreducedOutput=true -Pdoy=$doy -SbrdfMergedProduct=$brdfPcMosaicProduct -e -f NetCDF4-GA-ALBEDO -t $pcTarget"
    time $beamRootDir/bin/gpt-d-l3.sh ga.albedo.albedo -PreducedOutput=true -Pdoy=$doy -SbrdfMergedProduct=$brdfPcMosaicProduct -e -f NetCDF4-GA-ALBEDO -t $pcTarget
else
