...
JAVA_VERSION=/group_workspaces/cems/globalalbedo/soft/jdk1.7.0_60
...
do
    ...
    echo "$JAVA_VERSION/bin/java -Xms2g -Xmx8g -Djava.io.tmpdir=$GA_INST/tmp -jar $JARFILE ${resDegX[$r]}  ${resDegY[$r]}  ${resName[$r]}  ${div[$r]}  ${polRegion[$r]} $outfolder  $tile $pixCentre ${latMin[$r]} ${latMax[$r]} ${lonMin[$r]} ${lonMax[$r]}"
    $JAVA_VERSION/bin/java -Xms2g -Xmx8g -Djava.io.tmpdir=$GA_INST/tmp -jar $JARFILE ${resDegX[$r]}  ${resDegY[$r]}  ${resName[$r]}  ${div[$r]}  ${polRegion[$r]} $outfolder  $tile $pixCentre ${latMin[$r]} ${latMax[$r]} ${lonMin[$r]} ${lonMax[$r]}
done
