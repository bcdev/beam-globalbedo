...
m = PMonitor(inputs,
             request='ga-l3-tile-inversion-albedo-spectral-merge-uncertainties',
             logdir='log',
             hosts=[('localhost',32)], # let's try this number...
             types=[ ('ga-l3-tile-inversion-albedo-spectral-merge-uncertainties-step.sh',32)] )
...
for tile in tiles:
    ...
    for iyear in range(startYear, endYear+1):
        ...
        m.execute('ga-l3-tile-inversion-albedo-spectral-merge-uncertainties-step.sh',
                     ['uncerts'], 
                     [postCond], 
                     parameters=[tile,year,startDoy,endDoy,gaRootDir,spectralAlbedoRootDir,beamDir])
