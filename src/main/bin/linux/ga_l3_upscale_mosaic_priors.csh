...
if ( -e "$nosnowPriorRootDir" ) then
    time $beamRootDir/bin/gpt-d-l3.sh ga.l3.upscale.priors -Pscaling=$scaling -Pdoy=$Day -PcomputeSnow=false -PgaRootDir=$gaRootDir -PpriorRootDir=$nosnowPriorRootDir -e -t $TARGET
    set SUCCESS = $status
    echo "Status: $SUCCESS"
...
if ( -e "$snowPriorRootDir" ) then
    time $beamRootDir/bin/gpt-d-l3.sh ga.l3.upscale.priors -Pscaling=$scaling -Pdoy=$Day -PcomputeSnow=true -PgaRootDir=$gaRootDir -PpriorRootDir=$snowPriorRootDir -e -t $TARGET
    set SUCCESS = $status
    echo "Status: $SUCCESS"
    while ( ! -e $TARGET || $SUCCESS != 0)
        # repeat in case product was not written
