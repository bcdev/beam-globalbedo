...
if ( -e "$priorRootDir/PriorStage2" ) then
    time $beamRootDir/bin/gpt-d-l3.sh ga.l3.upscale.priors -Pscaling=$scaling -Pdoy=$Day -PcomputeSnow=false -PgaRootDir=$gaRootDir -PpriorRootDir=$priorRootDir -e -t $TARGET
    set SUCCESS = $status
    echo "Status: $SUCCESS"
...
if ( -e "$priorRootDir/PriorStage2Snow" ) then
    time $beamRootDir/bin/gpt-d-l3.sh ga.l3.upscale.priors -Pscaling=$scaling -Pdoy=$Day -PcomputeSnow=true -PgaRootDir=$gaRootDir -PpriorRootDir=$priorRootDir -e -t $TARGET
    set SUCCESS = $status
    echo "Status: $SUCCESS"
    while ( ! -e $TARGET || $SUCCESS != 0)
        # repeat in case product was not written
