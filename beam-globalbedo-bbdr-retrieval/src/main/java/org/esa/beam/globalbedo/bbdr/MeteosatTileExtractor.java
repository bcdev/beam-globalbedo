...
public class MeteosatTileExtractor extends Operator implements Output {
        ...
        for (int index = 0; index < tileCoordinates.getTileCount(); index++) {
            ...
            Callable<TileProduct> callable = new Callable<TileProduct>() {
                ...
                public TileProduct call() throws Exception {
                    Product reprojected = getReprojectedProductWithData(sourceProduct, tileName, sdrOnly);
    ...
    private void doExtract_simple() {
        for (int index = 0; index < tileCoordinates.getTileCount(); index++) {
            ...
            Product reproject = getReprojectedProductWithData(sourceProduct, tileName, sdrOnly);
            if (reproject != null) {
...
//        WriteOp writeOp = new WriteOp(product, file, DimapProductConstants.DIMAP_FORMAT_NAME);
    ...
    private static Product getReprojectedProductWithData(Product src,
