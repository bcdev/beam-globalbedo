...
public class UclCloudDetection {
    ...
    public boolean isCloud(float sdrRed, float sdrGreen, float sdrBlue) {
        ...
        float townCoefficient = townScatterData.getCoefficient(hsv);
        float landProbability = computeProbability(landCoefficient, cloudCoefficient);
        float townProbability = computeProbability(townCoefficient, cloudCoefficient);
        return landProbability > CLOUD_UNCERTAINTY_THRESHOLD || townCoefficient > CLOUD_UNCERTAINTY_THRESHOLD;
    }
